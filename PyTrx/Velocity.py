...
class Homography(ImageSequence):
    ...
    def __init__(
        self, imageList, camEnv, invmaskPath=None, calibFlag=True, band="L", equal=True
    ):
    ...
    def calcHomographies(
        self, params, homogmethod=cv2.RANSAC, ransacReprojThreshold=5.0
    ):
        ...
        for i in range(self.getLength() - 1):
            ...
            print(
                "\nProcessing homograpy for images: " + str(imn0) + " and " + str(imn1)
            )
            ...
            if params[0] == "sparse":
                ...
                hg = calcSparseHomography(
                    im0,
                    im1,
                    invmask,
                    [cameraMatrix, distortP],
                    homogmethod,
...
def calcSparseVelocity(
    ...
    # Seed features
    p0 = seedCorners(
        img1, mask, seedparams[0], seedparams[1], seedparams[2], min_features
    )

    # Track points between the image pair
    points, ptserrors = opticalMatch(img1, img2, p0, winsize, back_thresh, min_features)
    ...
    if homog is not None:
        ...
        dst_pts_homog = apply_persp_homographyPts(dst_pts_corr, hmatrix, inverse=True)
        ...
        print(
            str(dst_pts_corr.shape[0]) + " points remaining after homography correction"
        )
    ...
    if invprojvars is not None:
        ...
        for a, b in zip(xyzs, xyzd):
            xyzvel.append(
                np.sqrt((b[0] - a[0]) * (b[0] - a[0]) + (b[1] - a[1]) * (b[1] - a[1]))
            )
...
def calcDenseVelocity(
    ...
    pts, ptserrors = templateMatch(
        im0, im1, uv0, templatesize, searchsize, threshold, min_features, method
    )
    ...
    if calib is not None:
        ...
        # corrected, followed by those in image 2)
        # Correct points in first image
        src_pts_corr = cv2.undistortPoints(pts[0], calib[0], calib[1], P=newMat)

        # Correct points in second image
        dst_pts_corr = cv2.undistortPoints(pts[1], calib[0], calib[1], P=newMat)
...
def calcSparseHomography(
    ...
    if correct is not None:
        ...
        # Correct points in first image
        src_pts_corr = cv2.undistortPoints(points[0], correct[0], correct[1], P=newMat)

        # Correct tracked points in second image
        dst_pts_corr = cv2.undistortPoints(points[1], correct[0], correct[1], P=newMat)
    else:
        src_pts_corr = points[0]
        dst_pts_corr = points[1]

    # Find the homography between the two sets of corrected points
    homogMatrix, mask = cv2.findHomography(
        src_pts_corr, dst_pts_corr, method, ransacReprojThreshold
    )
    ...
    return (
        homogMatrix,
        [src_pts_corr, dst_pts_corr, homog_pts],
        ptserrors,
        homogerrors,
    )


def calcDenseHomography(
    img1,
    img2,
    mask,
    correct,
    ...
    dem=None,
    projvars=None,
    trackmethod=cv2.TM_CCORR_NORMED,
    homogmethod=cv2.RANSAC,
    ransacReprojThreshold=5.0,
    threshold=0.8,
    min_features=4,
):
    ...
    points, ptserrors = templateMatch(
        img1, img2, uv0, templatesize, searchsize, threshold, min_features, trackmethod
    )
    ...
    # Find the homography between the two sets of corrected points
    homogMatrix, mask = cv2.findHomography(
        src_pts_corr, dst_pts_corr, homogmethod, ransacReprojThreshold
    )
...
def apply_persp_homographyPts(pts, homog, inverse=False):
    ...
    if isinstance(pts, np.ndarray):
        ...
        for i in range(n):
            div = 1.0 / (
                homog[2][0] * pts[i][0][0] + homog[2][1] * pts[i][0][1] + homog[2][2]
            )
            hpts[i][0][0] = (
                homog[0][0] * pts[i][0][0] + homog[0][1] * pts[i][0][1] + homog[0][2]
            ) * div
            hpts[i][0][1] = (
                homog[1][0] * pts[i][0][0] + homog[1][1] * pts[i][0][1] + homog[1][2]
            ) * div
        return hpts
    ...
    # If input is as list
        ...
        for p in pts:
            div = 1.0 / (homog[2][0] * p[0] + homog[2][1] * p[1] + homog[2][2])
...
def opticalMatch(i0, iN, p0, winsize, back_thresh, min_features):
    ...
    p1, status1, error1 = cv2.calcOpticalFlowPyrLK(i0, iN, p0, None, **lk_params)

    # Track backwards from im1 to im0 using the forward-tracked points
    p0r, status0, error0 = cv2.calcOpticalFlowPyrLK(iN, i0, p1, None, **lk_params)
...
def templateMatch(
    ...
    for u, v in zip(uv0[:, :, 0], uv0[:, :, 1]):
        ...
        template = im0[
            int(v - (templatesize / 2)) : int(v + (templatesize / 2)),
            int(u - (templatesize / 2)) : int(u + (templatesize / 2)),
        ]
        search = im1[
            int(v - (searchsize / 2)) : int(v + (searchsize / 2)),
        ...
        if np.mean(resz) > threshold:
            ...
            loc_x = (u - ((resz.shape[1] * subpx) / 2)) + (location[0] * subpx)
            loc_y = v - ((resz.shape[1] * subpx) / 2) + (location[1] * subpx)
...
def readDEMmask(dem, img, invprojvars, demMaskPath=None):
    ...
    fig.canvas.manager.set_window_title(
        "Click to create mask. Press enter to record" " points."
    )
    ...
    uv = plt.ginput(
        n=0, timeout=0, show_clicks=True, mouse_add=1, mouse_pop=3, mouse_stop=2
    )
