...
class ExplicitRaster(object):
    ...
    def __init__(self, X, Y, Z, nodata=float("nan")):
        ...
        self._nodata = nodata
        self._extents = [
            X[0][0] - 0.5 * (X[0][1] - X[0][0]),
            X[-1][-1] + 0.5 * (X[-1][-1] - X[-1][-2]),
            Y[0][0] - 0.5 * (Y[1][0] - Y[0][0]),
            Y[-1][-1] + 0.5 * (Y[-1][-1] - Y[-2][-1]),
        ]
    ...
    def reportDEM(self):
        ...
        print(
            "Data Extent Coordinates are [xmin,xmax,ymin,ymax]: "
            + str(self.getExtent())
        )
...
def DEM_FromTiff(tiffFile):
    ...
    scanline = band.ReadRaster(
        0, 0, band.XSize, band.YSize, band.XSize, band.YSize, band.DataType
    )
