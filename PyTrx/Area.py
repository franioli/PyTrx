...
class Area(ImageSequence):
    ...
    def __init__(
        self, imageList, cameraenv, hmatrix, calibFlag=True, band="L", equal=True
    ):
    ...
    def calcAutoAreas(self, colour=False, verify=False):
        ...
        if colour is False:
            ...
            if self._colourrange is None:
                ...
                if self._calibFlag is True:
                    ...
                    setting = self._imageSet[self._maximg].getImageCorr(
                ...
                if self._enhance is not None:
                    setting = enhanceImage(
                        setting, self._enhance[0], self._enhance[1], self._enhance[2]
                    )

                # Define colour range
                self._colourrange = defineColourrange(
                    setting, setimn, pxplot=self._pxplot
                )
        ...
        for i in range(self.getLength()):
            ...
            if self._enhance is not None:
                img2 = enhanceImage(
                    img2, self._enhance[0], self._enhance[1], self._enhance[2]
                )
            ...
            if self._hmatrix is not None:
                ...
                out = calcAutoArea(
                    img2, imn, self._colourrange, None, self._threshold, invprojvars
                )
    ...
    def calcManualAreas(self):
        ...
        for i in range(self.getLength()):
            ...
            if self._calibFlag is True:
                img = self._imageSet[i].getImageCorr(
                    self._camEnv.getCamMatrixCV2(), self._camEnv.getDistortCoeffsCV2()
                )
            else:
            ...
            if self._hmatrix is not None:
                polys = calcManualArea(
                    img, imn, self._hmatrix[i], self._pxplot, invprojvars
                )
            else:
                polys = calcManualArea(img, imn, None, self._pxplot, invprojvars)
            area.append(polys)
    ...
    def verifyAreas(self, areas, invprojvars):
        ...
        for i in range(len(uvpts)):
            # Call corrected/uncorrected image
            if self._calibFlag is True:
                img1 = self._imageSet[i].getImageCorr(
                    self._camEnv.getCamMatrixCV2(), self._camEnv.getDistortCoeffsCV2()
                )
            else:
                img1 = self._imageSet[i].getImageArray()
            ...
            if 1:
                ...
                fig.canvas.manager.set_window_title(imn + ": Click on valid areas.")
    ...
    def setMax(self, maxMaskPath, maxim):
        ...
        if self._calibFlag is True:
            cameraMatrix = self._camEnv.getCamMatrixCV2()
            distortP = self._camEnv.getDistortCoeffsCV2()
            maxi = self._imageSet[maxim].getImageCorr(cameraMatrix, distortP)
        else:
...
def calcAutoArea(img, imn, colourrange, hmatrix=None, threshold=None, invprojvars=None):
    ...
    if version.parse(cv2.__version__) > version.parse("3.4.17"):
        line, hier = cv2.findContours(
            mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE
        )  # OpenCV v4 and higher
    else:
        i, line, hier = cv2.findContours(
            mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE
        )  # OpenCV v3.4.17 and lower
...
def calcManualArea(img, imn, hmatrix=None, pxplot=None, invprojvars=None):
    ...
    fig.canvas.manager.set_window_title(
        imn + ": Click around region. Press enter " "to record points."
    )
...
def defineColourrange(img, imn, pxplot=None):
    ...
    fig = plt.gcf()
    fig.canvas.manager.set_window_title(
        imn + ": Click lightest colour and darkest" " colour"
    )
    ...
    colours = plt.ginput(
        n=2, timeout=0, show_clicks=True, mouse_add=1, mouse_pop=3, mouse_stop=2
    )
