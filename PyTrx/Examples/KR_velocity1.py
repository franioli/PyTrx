...
try:
    ...
    from PyTrx.FileHandler import (
        writeHomogFile,
        writeVeloFile,
        writeVeloSHP,
        writeCalibFile,
    )
    from PyTrx.Utilities import (
        plotVeloPX,
        plotVeloXYZ,
        interpolateHelper,
        plotInterpolate,
...
camdata = "PyTrx/Examples/camenv_data/camenvs/CameraEnvironmentData_KR2_2014.txt"
camvmask = "PyTrx/Examples/camenv_data/masks/KR2_2014_vmask.jpg"
caminvmask = "PyTrx/Examples/camenv_data/invmasks/KR2_2014_inv.jpg"
camimgs = "PyTrx/Examples/images/KR2_2014_subset/*.JPG"


# Define data output directory
destination = "PyTrx/Examples/results/KR_velocity1/"
...
homog = Homography(
    camimgs, cameraenvironment, caminvmask, calibFlag=True, band="L", equal=True
)

# Calculate homography
hgout = homog.calcHomographies(
...
velo = Velocity(
    camimgs, cameraenvironment, hgout, camvmask, calibFlag=True, band="L", equal=True
)

velocities = velo.calcVelocities([vmethod, [mpt, ql, mdis], [vwinsize, bk, mfeat]])
...
for i in range(len(imn) - 1):
    ...
    plotVeloPX(
        ...
        show=True,
        save=target4 + "uv_" + imn[i],
    )

    # Plot xyz velocity points on dem
    print("Plotting XYZ output")
    plotVeloXYZ(
        xyzvel[i], xyz0[i], xyz1[i], dem, show=True, save=target4 + "xyz_" + imn[i]
    )
    ...
    plotInterpolate(
        grid, pointsextent, dem, show=True, save=target4 + "interp_" + imn[i]
    )
