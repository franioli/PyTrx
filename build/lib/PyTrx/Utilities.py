...
def plotGCPs(gcps, img, imn, dem, camloc, extent=None):
    ...
    fig.canvas.manager.set_window_title("GCP locations of " + str(imn))
    ...
    ax2.imshow(
        demred, origin="lower", extent=[lims[0], lims[1], lims[2], lims[3]], cmap="gray"
    )
...
def plotPrincipalPoint(camcen, img, imn):
    ...
    fig.canvas.manager.set_window_title("Principal Point of " + str(imn))
    ax1.axis([0, img.shape[1], img.shape[0], 0])
...
def plotCalib(matrix, distortion, img, imn):
    ...
    h = int(img.shape[0])
    w = int(img.shape[1])
    newMat, roi = cv2.getOptimalNewCameraMatrix(matrix, distortion, (w, h), 1, (w, h))

    # Correct image for distortion
    corr_image = cv2.undistort(img, matrix, distortion, newCameraMatrix=newMat)

    # Plot uncorrected and corrected images
    fig, (ax1, ax2) = plt.subplots(1, 2)
    fig.canvas.manager.set_window_title("Calibration output of " + str(imn))
...
def plotResiduals(img, ims, gcp1, gcp2, gcp3):
    ...
    fig.canvas.manager.set_window_title(
        "Average residual difference: " + str(np.nanmean(gcp3 - gcp2)) + " px"
    )
    ax1.axis([0, ims[1], ims[0], 0])
    ax1.imshow(img, cmap="gray")

    # Plot UV GCPs
    ax1.scatter(gcp1[:, 0], gcp1[:, 1], color="red", marker="+", label="UV")
...
def plotAreaPX(uv, img, show=True, save=None):
    ...
    # Set window name
    if save != None:
        fig.canvas.manager.set_window_title("UV output: " + save)
    else:
        fig.canvas.manager.set_window_title("UV output")
...
def plotVeloPX(uvvel, uv0, uv1, img, show=True, save=None):
    ...
    # Plot arrows
    xar, yar = arrowplot(
        uv0[:, 0, 0], uv0[:, 0, 1], uv1[:, 0, 0], uv1[:, 0, 1], scale=5.0, headangle=15
    )
...
def plotAreaXYZ(xyz, dem, show=True, save=None):
    ...
    # Set window name
    if save != None:
        fig.canvas.manager.set_window_title("XYZ output: " + save)
    else:
        fig.canvas.manager.set_window_title("XYZ output")
...
def plotVeloXYZ(xyzvel, xyz0, xyz1, dem, show=True, save=None):
    ...
    xar, yar = arrowplot(
        xyz0[:, 0], xyz0[:, 1], xyz1[:, 0], xyz1[:, 1], scale=5.0, headangle=15
    )
    ax1.plot(xar, yar, color="black", linewidth=1)
...
def plotInterpolate(grid, pointextent, dem, show=True, save=None):
    ...
    # Set name of window
    if save != None:
        fig.canvas.manager.set_window_title("XYZ interpolate: " + save)
    else:
        fig.canvas.manager.set_window_title("XYZ interpolate")
...
def interpolateHelper(xyzvel, xyz0, xyz1, method="linear"):
    ...
    for v, sx, ex, sy, ey in zip(
        xyzvel, xyz0[:, 0], xyz1[:, 0], xyz0[:, 1], xyz1[:, 1]
    ):
        if np.isnan(v) == False:
    ...
    grid_y, grid_x = np.mgrid[
        miny : maxy : complex(incrsy), minx : maxx : complex(incrsx)
    ]

    # Interpolate the velocity points to the grid
    grid = griddata(newpts, np.float64(velo), (grid_x, grid_y), method=method)
