...
class CamCalib(object):
    ...
    def __init__(self, *args):
        ...
        # Read calibration from several files
            ...
            # Calculate calibration from images
                ...
                print("\nAttempting to calculate camera calibs from input" + " images")
                calibimgs = []
                for i in sorted(glob.glob(args[0][0])):
                    calibimgs.append(i)

                arg, err = calibrateImages(
                    calibimgs, [int(args[0][1]), int(args[0][2])]
                )
                arg = self.checkMatrix(arg)
    ...
    def getDistortCoeffsCV2(self):
        ...
        if len(self._radCorr) == 2:
            ...
            return np.append(
                np.append(self._radCorr[0:2], self._tanCorr), self._radCorr[2:]
            )
    ...
    def getCamMatrixCV2(self):
        ...
        if self._intrMatCV2 is None:
            ...
            if (
                self._intrMat[2, 0] != 0
                and self._intrMat[2, 1] != 0
                and self._intrMat[0, 2] == 0
                and self._intrMat[1, 2] == 0
            ):
            ...
            for i in range(4):
                x = it[i, 0]
                y = it[i, 1]
                self._intrMatCV2[x, y] = 0.0
            self._intrMatCV2[2, 2] = 1.0
...
class CamEnv(CamCalib):
    ...
    def dataFromFile(self, filename):
        ...
        if lineNo != None:
            ...
            print("\nDem densification level not supplied in: " + str(filename))
    ...
    def optimiseCamEnv(self, optimise, optmethod="trf", show=False):
        ...
        opt_projvars = optimiseCamera(optimise, projvars, xyz, uv, optmethod, show)
    ...
    def reportCamData(self):
        ...
        # Reference image
        print("\nReference image used for baseline homography " + "and/or GCP control:")
...
def calibrateImages(imageFiles, xy, refine=None):
    ...
    for fname in imageFiles:
        ...
        patternFound, corners = cv2.findChessboardCorners(gray, (xy[1], xy[0]), None)
...
def setProjection(
    dem, camloc, camdir, radial, tangen, foclen, camcen, refimg, viewshed=True
):
    ...
    XYZ = np.column_stack([X[visible[:]], Y[visible[:]], Z[visible[:]]])
    uv0, dummy, inframe = projectXYZ(
        camloc, camdir, radial, tangen, foclen, camcen, refimg, XYZ
    )
...
def projectXYZ(camloc, camdirection, radial, tangen, foclen, camcen, refimg, xyz):
    ...
    if False:
        ...
        # Transposed from ImGRAFT
        if not np.allclose(radial[2:6], [0.0, 0.0, 0.0, 0.0]):
            a = 1.0 + radial[0] * r2 + radial[1] * r2 * r2 + radial[2] * r2 * r2 * r2
            a = a / (
                1.0 + radial[3] * r2 + radial[4] * r2 * r2 + radial[5] * r2 * r2 * r2
            )
        else:
            a = 1.0 + radial[0] * r2 + radial[1] * r2 * r2 + radial[2] * r2 * r2 * r2
        ...
        pt1 = (
            a * xy[:, 0]
            + 2 * tangen[0] * xty
            + tangen[1] * (r2 + 2 * xy[:, 0] * xy[:, 0])
        )
        pt2 = (
            a * xy[:, 1]
            + 2 * tangen[0] * xty
            + tangen[1] * (r2 + 2 * xy[:, 1] * xy[:, 1])
        )
...
def optimiseCamera(optimise, projvars, GCPxyz, GCPuv, optmethod="trf", show=False):
    ...
    res0 = computeResidualsUV(None, stable, GCPxyz, GCPuv, refimg, optimise=None)
    GCPxyz_proj0, depth, inframe = projectXYZ(
        camloc, campose, radcorr, tancorr, focal, camcen, refimg, GCPxyz
    )
    ...
    if optimise == "YPR":
        ...
        params = np.concatenate(
            (radcorr.flatten(), tancorr.flatten(), np.array(focal), np.array(camcen))
        )
    ...
    if out.success is True:
        ...
        GCPxyz_proj1, depth, inframe = projectXYZ(
            camloc, campose, radcorr, tancorr, focal, camcen, refimg, GCPxyz
        )
        ...
        projvars1 = [camloc, campose, radcorr, tancorr, focal, camcen, refimg]
...
def computeResidualsXYZ(invprojvars, GCPxyz, GCPuv, dem):
    ...
    fig.canvas.manager.set_window_title(
        "Average residual difference: " + str(np.nanmean(residual)) + " m"
    )
    ...
    # Plot UV GCPs
    ax1.scatter(GCPxyz[:, 0], GCPxyz[:, 1], color="red", marker="+", label="XYZ")
...
def computeResidualsUV(params, stable, GCPxyz, GCPuv, refimg, optimise="YPR"):
    """Function for computing the pixel difference between GCP image points
    and GCP projected XYZ points. This function is used in the optimisation
    function (optimiseCamera), with parameters for optimising defined in the
    first variable and stable parameters defined in the second. If no
    ...
    GCPxyz_proj, depth, inframe = projectXYZ(
        camloc, campose, radcorr, tancorr, focal, camcen, refimg, GCPxyz
    )
